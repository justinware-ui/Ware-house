  ...
  return (
    <div className={containerClass}>
      <div ref={scrollWrapRef} data-scroll-area className={mode === 'full' ? 'relative overflow-hidden' : 'relative flex-1 overflow-hidden'} style={mode === 'full' ? { height: effectiveHasSent && scrollMaxH ? scrollMaxH : undefined, maxHeight: 'calc(100vh - 340px)' } : undefined}>
        <div className="absolute top-0 left-0 right-0 h-3 z-10 pointer-events-none transition-opacity duration-200" style={{ background: 'radial-gradient(ellipse 70% 100% at 50% 0%, rgba(0,0,0,0.2) 0%, transparent 100%)', opacity: canScrollUp ? 1 : 0 }} />
        <div className="absolute bottom-0 left-0 right-0 h-3 z-10 pointer-events-none transition-opacity duration-200" style={{ background: 'radial-gradient(ellipse 70% 100% at 50% 100%, rgba(0,0,0,0.2) 0%, transparent 100%)', opacity: canScrollDown ? 1 : 0 }} />
        ...
        {showFeedbackInput && (
          <div className="ml-10 flex gap-2">
            <input
              ...
              className="flex-1 text-sm border border-[#3D3834] bg-[#2A2622] text-[#F5F0EB] rounded-lg px-3 py-2 outline-none focus:border-brand-400"
            />
            <button
              onClick={() => {
      ...
      <div
        ...
        className={
          mode === 'full'
            ...
            : 'border-t border-[#3D3834] px-4 py-3 bg-[#2A2622]'
        }
        style={
      ...
      >
        <div
          className="bg-[#2A2622] rounded-2xl flex flex-col transition-shadow duration-200"
          style={{
            ...
            border: inputFocused ? '2px solid #FF9356' : '1px solid #4A4540',
            boxShadow: inputFocused ? '0 0 0 5px rgba(255, 150, 89, 0.5)' : '0 1px 2px 0 rgba(0, 0, 0, 0.2)',
        ...
        >
          <div className="flex-1 px-4 pt-3 pb-1">
            <textarea
              ...
              className="w-full resize-none outline-none text-sm text-[#F5F0EB] placeholder:text-[#78716C] bg-transparent overflow-hidden"
          ...
          <div className="flex items-center justify-between px-4 pb-3">
            <button className="text-[#78716C] hover:text-[#A8A29E] transition-colors">
              <Mic size={18} />
            </button>
            <button
              onClick={handleSend}
              className="transition-colors"
              style={{ color: input.trim() ? '#FF9356' : '#d1d5db' }}
            >
              <Send size={18} />
            </button>
          </div>
...
function renderInlineMarkdown(text: string) {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[0] === '📚') {
      ...
      parts.push(
        <a key={match.index} href={match[3]} target="_blank" rel="noopener noreferrer" className="text-[#FF9356] underline hover:opacity-80 transition-opacity">
          {match[2]}
        </a>,
      )
    }
...
function UserMessage({ msg }: { msg: ChatMessage }) {
  return (
    <div className="flex flex-col items-end gap-1">
      <div className="rounded-2xl px-4 py-3 text-sm max-w-[85%] whitespace-pre-wrap" style={{ backgroundColor: '#3D2A1E', color: '#F5F0EB', border: '1px solid #5C3D2A' }}>
        {msg.content}
      </div>
      {msg.actions && (
  ...
  return (
    <div className="flex gap-3 items-start">
        ...
        <svg width="28" height="28" viewBox="14 8 62 62" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <defs>
            <linearGradient id="paint_ai_msg" x1="19.0464" y1="40.0309" x2="69.4464" y2="40.0309" gradientUnits="userSpaceOnUse">
              <stop stopColor="#FFB352"/><stop offset="0.5" stopColor="#FF9356"/><stop offset="1" stopColor="#EB2E24"/>
            </linearGradient>
          </defs>
        </svg>
      </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-2">
        {/* Thinking indicator */}
        {msg.thinking && (
          <div className="flex items-center gap-2 text-sm" style={{ color: '#FF9356' }}>
            <div className="w-4 h-4 border-2 border-orange-200 rounded-full animate-spin" style={{ borderTopColor: '#FF9356' }} />
            Thinking...
          </div>
        )}
        ...
        {msg.toolGroups && msg.toolGroups.length > 0 && (
          <div className="rounded-xl border border-[#3D3834] bg-[#302C28] p-3 text-xs overflow-hidden">
              ...
              return (
                <div key={key} className="mb-2 last:mb-0">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-[#A8A29E] hover:text-[#F5F0EB] transition-colors w-full text-left"
                  >
                    {isOpen ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                    {allDone ? (
                      <span style={{ color: '#FF9356' }}>●</span>
                    ) : (
                      <div className="w-3 h-3 border-2 border-orange-200 rounded-full animate-spin" style={{ borderTopColor: '#FF9356' }} />
                    )}
                    <span className="font-medium">{group.label} ({completedCount}/{group.tools.length})</span>
                    {!allDone && <span className="ml-auto text-[#78716C] text-[10px]">Running...</span>}
                  </button>
                  {isOpen && (
                      ...
                      {group.tools.map((tool, ti) => (
                        <div key={ti} className="flex items-start gap-1.5">
                          {tool.status === 'complete' ? (
                            <span className="mt-px" style={{ color: '#FF9356' }}>✓</span>
                          ) : (
                            <div className="w-3 h-3 border-2 border-orange-200 rounded-full animate-spin mt-px" style={{ borderTopColor: '#FF9356' }} />
                          )}
                          <div>
                            <span className="font-medium text-[#F5F0EB]">{tool.name}</span>
                            <span className="text-[#78716C] ml-1">{tool.description}</span>
                            {tool.result && tool.status === 'complete' && (
                              <div className="text-[#78716C] ml-4">{tool.result}</div>
                            )}
                          </div>
                        </div>
                      ))}
        ...
        {/* Main content */}
        {msg.content && (
          <div className="text-sm text-[#F5F0EB] whitespace-pre-wrap">{renderInlineMarkdown(msg.content)}</div>
        )}
...
const confidenceColor: Record<ConfidenceLevel, { icon: string; border: string; bg: string; text: string }> = {
  high: { icon: '#1a7a4c', border: '#2d5c3f', bg: '#1a2e22', text: '#6ee7b7' },
  medium: { icon: '#FF9356', border: '#5C3D2A', bg: '#2e1f16', text: '#fdba74' },
  low: { icon: '#dc2626', border: '#5C2A2A', bg: '#2e1616', text: '#fca5a5' },
}
...
function ContentCard({
  ...
  return (
    <div className="rounded-xl border border-[#3D3834] bg-[#302C28] overflow-visible">
      <div className="flex items-center gap-3 px-3 py-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-[#F5F0EB] truncate">{match.demo.title}</p>
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-xs text-[#A8A29E]">{match.demo.creator}</span>
            <span className="text-xs text-[#FF9356] font-semibold">Show more</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 shrink-0">
            ...
            >
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="group-hover/info:[&_path]:[fill:var(--info-hover)]">
                ...
                <g mask={`url(#mask_info_${infoKey})`}>
                  <path d="M9.99935 14.1666C10.2355 14.1666 10.4335 14.0866 10.5935 13.9266C10.753 13.7672 10.8327 13.5694 10.8327 13.3333V9.97913C10.8327 9.74301 10.753 9.54857 10.5935 9.39579C10.4335 9.24301 10.2355 9.16663 9.99935 9.16663C9.76324 9.16663 9.56546 9.24635 9.40602 9.40579C9.24602 9.56579 9.16602 9.76385 9.16602 9.99996V13.3541C9.16602 13.5902 9.24602 13.7847 9.40602 13.9375C9.56546 14.0902 9.76324 14.1666 9.99935 14.1666ZM9.99935 7.49996C10.2355 7.49996 10.4335 7.41996 10.5935 7.25996C10.753 7.10051 10.8327 6.90274 10.8327 6.66663C10.8327 6.43051 10.753 6.23246 10.5935 6.07246C10.4335 5.91301 10.2355 5.83329 9.99935 5.83329C9.76324 5.83329 9.56546 5.91301 9.40602 6.07246C9.24602 6.23246 9.16602 6.43051 9.16602 6.66663C9.16602 6.90274 9.24602 7.10051 9.40602 7.25996C9.56546 7.41996 9.76324 7.49996 9.99935 7.49996ZM9.99935 18.3333C8.84657 18.3333 7.76324 18.1144 6.74935 17.6766C5.73546 17.2394 4.85352 16.6458 4.10352 15.8958C3.35352 15.1458 2.7599 14.2638 2.32268 13.25C1.8849 12.2361 1.66602 11.1527 1.66602 9.99996C1.66602 8.84718 1.8849 7.76385 2.32268 6.74996C2.7599 5.73607 3.35352 4.85413 4.10352 4.10413C4.85352 3.35413 5.73546 2.76024 6.74935 2.32246C7.76324 1.88524 8.84657 1.66663 9.99935 1.66663C11.1521 1.66663 12.2355 1.88524 13.2493 2.32246C14.2632 2.76024 15.1452 3.35413 15.8952 4.10413C16.6452 4.85413 17.2388 5.73607 17.676 6.74996C18.1138 7.76385 18.3327 8.84718 18.3327 9.99996C18.3327 11.1527 18.1138 12.2361 17.676 13.25C17.2388 14.2638 16.6452 15.1458 15.8952 15.8958C15.1452 16.6458 14.2632 17.2394 13.2493 17.6766C12.2355 18.1144 11.1521 18.3333 9.99935 18.3333ZM9.99935 16.6666C11.8466 16.6666 13.4196 16.0175 14.7185 14.7191C16.0168 13.4202 16.666 11.8472 16.666 9.99996C16.666 8.15274 16.0168 6.57968 14.7185 5.28079C13.4196 3.98246 11.8466 3.33329 9.99935 3.33329C8.15213 3.33329 6.57935 3.98246 5.28102 5.28079C3.98213 6.57968 3.33268 8.15274 3.33268 9.99996C3.33268 11.8472 3.98213 13.4202 5.28102 14.7191C6.57935 16.0175 8.15213 16.6666 9.99935 16.6666Z" fill={isInfoOpen ? colors.icon : '#F5F0EB'}/>
                </g>
              </svg>
            </button>
          ...
          <div className="relative group/add flex items-center">
            ...
            {isSelected ? (
              ...
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="12" cy="12" r="10" stroke="#FF9356" strokeWidth="1.5"/>
                <path d="M12 8v8M8 12h8" stroke="#FF9356" strokeWidth="1.5" strokeLinecap="round"/>
              </svg>
            )}
            </button>
  ...
  return (
    <div className="flex flex-col gap-3 mt-2">
      ...
      {proposal.template !== 'single_asset' && (
        <div className="bg-[#302C28] rounded-xl p-3 border border-[#3D3834]">
          <div className="text-xs font-semibold text-[#A8A29E] uppercase tracking-wider mb-1">Discovery Question</div>
          <div className="text-sm text-[#F5F0EB]">&ldquo;{proposal.discoveryQuestion}&rdquo;</div>
          <div className="text-xs text-[#A8A29E] mt-2">Each persona below will become an answer branch. Select the content you want under each.</div>
        </div>
      )}
      ...
      {proposal.personas.map((persona, pi) => (
        <div key={pi}>
          <div className="px-1 mb-2">
            <div className="text-sm font-semibold text-[#F5F0EB]">{persona.name}</div>
          ...
          {proposal.template === '2_disco_branch' && proposal.secondDiscoveryQuestions && (
            <div className="bg-[#302C28] rounded-xl p-3 border border-[#3D3834] mb-2">
              <div className="text-xs font-semibold text-[#A8A29E] uppercase tracking-wider mb-1">
          ...
          {persona.noMatchReason && (
            <div className="mb-2 px-3 py-2 bg-[#2e2516] border border-[#5C4D2A] rounded-xl text-xs text-[#fbbf24] flex items-start gap-2">
              ...
              {persona.painPointMatches.map((ppGroup, gi) => (
                <div key={gi}>
                  <div className="px-1 mb-1.5">
                    <div className="text-xs font-semibold text-[#A8A29E] uppercase tracking-wider">{ppGroup.painPoint}</div>
                  </div>
